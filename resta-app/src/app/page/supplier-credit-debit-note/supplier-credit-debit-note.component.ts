// supplier-credit-debit-note.component.ts
...
import {
  CreateSupplierCreditDebitNoteDto,
  SupplierCreditDebitNote,
  SupplierCreditDebitNoteService,
  UpdateSupplierCreditDebitNoteDto
} from '../../services/supplier-credit-debit-note.service';
...
function emptyForm(): SupplierCreditDebitNote {
  return {
    supplierCreditDebitNoteId: 0,
    voucherNo:                 '',
...
@Component({
  selector: 'app-supplier-credit-debit-note',
  imports: [AppModule, GridReportToolbarComponent],
  templateUrl: './supplier-credit-debit-note.component.html',
  styleUrls: ['./supplier-credit-debit-note.component.scss'],
  providers: [ConfirmationService, MessageService]
})
export class SupplierCreditDebitNoteComponent implements OnInit, OnDestroy {

  notes: SupplierCreditDebitNote[] = [];
  selectedNote: SupplierCreditDebitNote | null = null;
  isLoading = false;
  totalRecords = 0;
  ...
  formData: SupplierCreditDebitNote = emptyForm();
  formErrors: FormErrors = {};
  isSaving = false;

  supplierOptions: SelectOption<number>[] = [];
  ...
  constructor(
    private noteService: SupplierCreditDebitNoteService,
  ...
  get dialogTitle(): string {
    return this.dialogMode === 'add'
      ? 'Add Supplier Credit / Debit Note'
      : 'Edit Supplier Credit / Debit Note';
  }
  ...
  private loadSuppliers(): void {
    this.supplierService.getAll({ pageSize: 500 }).subscribe({
      next: (res: any) => {
        ...
        this.supplierOptions = raw.map((x: any) => {
          const id = x.supplierId ?? x.id ?? 0;
          const name = x.name ?? `Supplier #${id}`;
    ...
    this.noteService.getAll({ search, pageSize: 200 }).subscribe({
      ...
      error: () => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Load Failed',
          detail: 'Could not load supplier credit/debit notes. Please try again.'
        });
      }
    });
  }

  private mapNote(x: any): SupplierCreditDebitNote {
    return {
      supplierCreditDebitNoteId: Number(x.supplierCreditDebitNoteId ?? x.id ?? 0),
  ...
  openEditDialog(note: SupplierCreditDebitNote): void {
  ...
  private toCreateDto(f: SupplierCreditDebitNote): CreateSupplierCreditDebitNoteDto {
    return {
  ...
  private toUpdateDto(f: SupplierCreditDebitNote): UpdateSupplierCreditDebitNoteDto {
    return {
      voucherNo:   f.voucherNo || null,
      voucherDate: f.voucherDate,
      voucherType: f.voucherType,
    ...
    this.noteService.create(this.toCreateDto(this.formData)).subscribe({
      next: (res: any) => {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Note Added',
          detail: `Note "${added.voucherNo || added.supplierCreditDebitNoteId}" was added successfully.`
        });
      },
      error: () => {
        this.isSaving = false;
  ...
  private saveEdit(): void {
    this.noteService.update(
      this.formData.supplierCreditDebitNoteId,
      this.toUpdateDto(this.formData)
    ).subscribe({
      next: () => {
        const idx = this.notes.findIndex(
          n => n.supplierCreditDebitNoteId === this.formData.supplierCreditDebitNoteId
        );
        if (idx !== -1) {
          this.notes = [
        ...
        if (this.selectedNote?.supplierCreditDebitNoteId === this.formData.supplierCreditDebitNoteId) {
        ...
        this.messageService.add({
          ...
          detail: `Note "${this.formData.voucherNo || this.formData.supplierCreditDebitNoteId}" was updated successfully.`
        });
      },
      error: () => {
        this.isSaving = false;
  ...
  confirmDelete(note: SupplierCreditDebitNote, event: Event): void {
    event.stopPropagation();
    const label = note.voucherNo || `#${note.supplierCreditDebitNoteId}`;
  ...
  private executeDelete(note: SupplierCreditDebitNote): void {
    this.noteService.delete(note.supplierCreditDebitNoteId).subscribe({
      next: () => {
        this.notes = this.notes.filter(
          n => n.supplierCreditDebitNoteId !== note.supplierCreditDebitNoteId
        );
        if (this.selectedNote?.supplierCreditDebitNoteId === note.supplierCreditDebitNoteId) {
        ...
        this.messageService.add({
          ...
          detail: `Note "${note.voucherNo || note.supplierCreditDebitNoteId}" was deleted.`
        });
      },
      error: () => {
        this.messageService.add({
  ...
  get noteReportConfig(): GridReportConfig {
    return {
      title: 'Supplier Credit / Debit Notes',
      subtitle: this.hasActiveSearch ? `Search: ${this.searchTerm}` : undefined,
      fileName: 'supplier_credit_debit_notes',
      ...
      columns: [
        ...
        },
        {
          header: 'Supplier',
          field: 'supplierId',
          format: (v) => this.getSupplierName(v as number | null)
        },
